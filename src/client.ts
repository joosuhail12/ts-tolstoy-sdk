...
import {
  ...
  ExecutionLogRetrieveResponse,
  ExecutionLogUpdateParams,
  ExecutionLogs,
...
import {
  ...
  WebhookGetValidEventTypesResponse,
  WebhookListParams,
  WebhookListResponse,
...
export interface ClientOptions {
   ...
   * - `production` corresponds to `https://tolstoy.getpullse.com`
   ...
   *
   * Defaults to process.env['TOLSTOY_API_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['TOLSTOY_API_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Tolstoy API API.
 */
export class TolstoyAPI {
  orgID: string;
  userID: string;
   ...
   * API Client for interfacing with the Tolstoy API API.
   *
   * @param {string | undefined} [opts.orgID=process.env['TOLSTOY_API_ORG_ID'] ?? undefined]
   * @param {string | undefined} [opts.userID=process.env['TOLSTOY_API_USER_ID'] ?? undefined]
   * @param {Environment} [opts.environment=production] - Specifies the environment URL to use for the API.
   * @param {string} [opts.baseURL=process.env['TOLSTOY_API_BASE_URL'] ?? https://tolstoy.getpullse.com] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('TOLSTOY_API_BASE_URL'),
    orgID = readEnv('TOLSTOY_API_ORG_ID'),
    userID = readEnv('TOLSTOY_API_USER_ID'),
    ...opts
  }: ClientOptions = {}) {
    if (orgID === undefined) {
      throw new Errors.TolstoyAPIError(
        "The TOLSTOY_API_ORG_ID environment variable is missing or empty; either provide it, or instantiate the TolstoyAPI client with an orgID option, like new TolstoyAPI({ orgID: 'My Org ID' }).",
      );
    }
    if (userID === undefined) {
      throw new Errors.TolstoyAPIError(
        "The TOLSTOY_API_USER_ID environment variable is missing or empty; either provide it, or instantiate the TolstoyAPI client with an userID option, like new TolstoyAPI({ userID: 'My User ID' }).",
      );
    }
    ...
    if (baseURL && opts.environment) {
      throw new Errors.TolstoyAPIError(
        'Ambiguous URL; The `baseURL` option (or TOLSTOY_API_BASE_URL env var) and the `environment` option are given. If you want to use the environment you must pass baseURL: null',
      );
    }

    this.baseURL = options.baseURL || environments[options.environment || 'production'];
    this.timeout = options.timeout ?? TolstoyAPI.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('TOLSTOY_API_LOG'), "process.env['TOLSTOY_API_LOG']", this) ??
      defaultLogLevel;
   ...
   * @example
   * ```ts
   * const response = await client.getWelcomeMessage();
   * ```
   */
  getWelcomeMessage(options?: RequestOptions): APIPromise<string> {
  ...
  protected async authHeaders(opts: FinalRequestOptions): Promise<NullableHeaders | undefined> {
    return buildHeaders([await this.xOrgIDAuth(opts), await this.xUserIDAuth(opts)]);
  }

  protected async xOrgIDAuth(opts: FinalRequestOptions): Promise<NullableHeaders | undefined> {
    return buildHeaders([{ 'x-org-id': this.orgID }]);
  }

  protected async xUserIDAuth(opts: FinalRequestOptions): Promise<NullableHeaders | undefined> {
    return buildHeaders([{ 'x-user-id': this.userID }]);
  }
  ...
  static TolstoyAPI = this;
  static DEFAULT_TIMEOUT = 60000; // 1 minute

  static TolstoyAPIError = Errors.TolstoyAPIError;
...
TolstoyAPI.Tools = Tools;
TolstoyAPI.Actions = Actions;
