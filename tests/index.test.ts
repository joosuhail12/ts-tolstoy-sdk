...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['TOLSTOY_API_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: TolstoyAPI) => {
    ...
    test('default logLevel is warn', async () => {
      const client = new TolstoyAPI({ orgID: 'My Org ID', userID: 'My User ID' });
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      process.env['TOLSTOY_API_LOG'] = 'debug';
      const client = new TolstoyAPI({ logger: logger, orgID: 'My Org ID', userID: 'My User ID' });
    ...
    test('warn when env var level is invalid', async () => {
      ...
      process.env['TOLSTOY_API_LOG'] = 'not a log level';
      const client = new TolstoyAPI({ logger: logger, orgID: 'My Org ID', userID: 'My User ID' });
      expect(client.logLevel).toBe('warn');
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'TOLSTOY_API_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
  ...
  describe('baseUrl', () => {
    ...
    afterEach(() => {
      process.env['TOLSTOY_API_BASE_URL'] = undefined;
    });
    ...
    test('env variable', () => {
      process.env['TOLSTOY_API_BASE_URL'] = 'https://example.com/from_env';
      const client = new TolstoyAPI({ orgID: 'My Org ID', userID: 'My User ID' });
      expect(client.baseURL).toEqual('https://example.com/from_env');
    });

    test('empty env variable', () => {
      process.env['TOLSTOY_API_BASE_URL'] = ''; // empty
      const client = new TolstoyAPI({ orgID: 'My Org ID', userID: 'My User ID' });
      expect(client.baseURL).toEqual('https://tolstoy.getpullse.com');
    });

    test('blank env variable', () => {
      process.env['TOLSTOY_API_BASE_URL'] = '  '; // blank
      const client = new TolstoyAPI({ orgID: 'My Org ID', userID: 'My User ID' });
      expect(client.baseURL).toEqual('https://tolstoy.getpullse.com');
    });
    ...
    test('env variable with environment', () => {
      ...
      expect(
        () => new TolstoyAPI({ orgID: 'My Org ID', userID: 'My User ID', environment: 'production' }),
    ...
    test('in request options overridden by env variable', () => {
      process.env['TOLSTOY_API_BASE_URL'] = 'http://localhost:5000/env';
      const client = new TolstoyAPI({ orgID: 'My Org ID', userID: 'My User ID' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
  ...
  test('maxRetries option is correctly set', () => {
    const client = new TolstoyAPI({ maxRetries: 4, orgID: 'My Org ID', userID: 'My User ID' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new TolstoyAPI({ orgID: 'My Org ID', userID: 'My User ID' });
  ...
  test('with environment variable arguments', () => {
    ...
    process.env['TOLSTOY_API_ORG_ID'] = 'My Org ID';
    process.env['TOLSTOY_API_USER_ID'] = 'My User ID';
    const client = new TolstoyAPI();
  ...
  test('with overridden environment variable arguments', () => {
    // set options via env var
    process.env['TOLSTOY_API_ORG_ID'] = 'another My Org ID';
    process.env['TOLSTOY_API_USER_ID'] = 'another My User ID';
    const client = new TolstoyAPI({ orgID: 'My Org ID', userID: 'My User ID' });
    expect(client.orgID).toBe('My Org ID');
    expect(client.userID).toBe('My User ID');
  });
...
describe('retries', () => {
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new TolstoyAPI({ orgID: 'My Org ID', userID: 'My User ID', fetch: testFetch });
